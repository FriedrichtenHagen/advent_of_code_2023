# new individual card order: AKQT98765432J
...
def x_of_a_kind(hand, x):
    ...
    # if J exists: read out amount
    if('J' in char_count.keys()):
        j_amount = char_count['J']
        if(j_amount == 5):
            # replace J with A AAAAA

        else:
            # remove J from dict
            char_count.pop('J', None)
        # find char with highest count
        max_key = max(char_count, key=char_count.get)
        # move amount to highest value
        char_count[f'{max_key}'] += j_amount
        # if several highest values are tied, choose first one
        # or should the highest alphabetical key be chosen? 
...
with open("/Users/friedrichtenhagen/coding/advent_of_code_2023/day7/input.txt") as f:
    ...
    print(find_type('JkkkJ'))
    print(find_type('Jkkkl'))
    print(find_type('Jkikl'))
    print(find_type('Jffudd'))
    print(find_type('Jkydd'))
    print(find_type('Jtikl'))
    print(find_type('12345'))
    print(find_type('Jky5d'))
